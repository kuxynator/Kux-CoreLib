...
function tests.removeGaps()
	...
	assert(That.IsReferenceEqual(r, t))
	assert(That.IsEqual(#r, length))
...
function tests.table_nil_test1()
	local t = {"A","B",nil,"D"}
	assert(That.IsEqual(#t, 4))
	t[3]=nil	-- {"A","B",nil,"D"}
	assert(That.IsEqual(#t, 4))
	t[2]=nil	-- {"A",nil,nil,"D"}
	assert(That.IsEqual(#t, 4))
	t[4]=nil	-- {"A"}
	assert(That.IsEqual(#t, 1))

	t = {"A","B",nil,"D"}
	table.remove(t,3)	-- {"A","B","D"}
	assert(That.IsEqual(#t,3))
	assert(That.IsEqual(t[3],"D"))

	table.insert(t,3,nil) -- {"A","B",nil,"D"}
	assert(That.IsEqual(#t, 4))
	assert(That.IsEqual(t[4],"D"))
...
function tests.table_bug_1_gap_array_set_value_of_last_index_to_nil()
	...
	t[2] = nil
	t[4] = nil
	assert(That.IsEqual(#t, 5))
	t[5] = nil -- set last index to nil
	assert(That.IsEqual(#t, 1)) -- but should be 5 or 3

	assert(That.IsEqual(pcall(function ()t:remove(4)	end), false))
end

function tests.table_create_gap()
	local t = {"A",nil,"C"}
	assert(That.IsEqual(#t, 3))	-- as expected {"A",nil,"C"}
	assert(That.IsEqual(t[3], "C"))
end
function tests.table_create_gapend()
	local t = {"A",nil,nil}
	assert(That.IsEqual(#t, 1))	-- but is {"A"}
end
...
function tests.size_gaparray()
	assert(That.IsEqual(#{"A",nil,"C"}, 3)) -- nil is counted
	assert(That.IsEqual(#{"A","B",nil}, 2)) -- nil is counted exept at end
	assert(That.IsEqual(Table.count({"A",nil,"C"}), 2))
	assert(That.IsEqual(Table.count({"A","B",nil}), 2))
end

function tests.size_array()
	local sample={"A","B","C"}
	assert(That.IsEqual(Table.count(sample), 3))
end

function tests.size_arraygaps()
	local sample={"A","B","C","D"}
	sample[2]=nil
	assert(That.IsNil(sample[2]))
	assert(That.IsEqual(Table.count(sample), 3))
end

function tests.size_dictionary()
	local sample={k1="A",k2=nil,k3="C"}
	assert(That.IsEqual(Table.count(sample), 2))
end
...
function tests.createPatch()
	...
	assert(That.IsEqual(r.a, nil)) -- value not changed
	assert(That.IsEqual(r.b, 3))
	assert(That.IsEqual(r.c, "§DELETED"))
	assert(That.IsEqual(r.s, nil)) -- table not changed
	assert(That.IsEqual(r.sa,"§DELETED"))
	assert(That.IsEqual(r.sb, t2.sb))
	--print(serpent.block(r))
end

function tests.isEmpty()
	local t = {}; assert(That.IsTrue(Table.isEmpty(t)))
	t = {"a","b"}; 	assert(That.IsFalse(Table.isEmpty(t)))
	t[1]=nil; assert(That.IsFalse(Table.isEmpty(t)))
...
function tests.isEqual()
	local t1={};local t2={}
	assert(That.IsTrue(Table.isEqual(t1,t2)))
	t1={1,2,3};t2={1,2,3}; assert(That.IsTrue(Table.isEqual(t1,t2)))			-- array
	t1={1,2,{3,4}};t2={1,2,{3,4}}; assert(That.IsTrue(Table.isEqual(t1,t2)))	-- array, recursive

	t1={a=1,b=2};t2={a=1,b=2}; assert(That.IsTrue(Table.isEqual(t1,t2)))			-- dictionary
	t1={a=1,b={c=3}};t2={a=1,b={c=3}}; assert(That.IsTrue(Table.isEqual(t1,t2)))	-- dictionary, recursive
	t1={a=1,b=2};t2={b=2,a=1}; assert(That.IsTrue(Table.isEqual(t1,t2)))	-- dictionary, recursive, swpapped

	t1={1,2,3};t2={1,2}; assert(That.IsFalse(Table.isEqual(t1,t2)))
	t1={1,2,3};t2={1,2,4}; assert(That.IsFalse(Table.isEqual(t1,t2)))
	t1={1,2,3};t2={3,2,1}; assert(That.IsFalse(Table.isEqual(t1,t2)))
...
function tests.diff()
	...
	local b = {a=1,b={c=2}}
	Table.diff(a,b)
	assert(That.IsTrue(Table.isEmpty(a)))
end

function tests.hasIndices()
	assert(That.IsTrue(Table.hasIndices({1,2,3})))
	assert(That.IsTrue(Table.hasIndices({k="x", 1,2,3})))

	local t={}; t[1]=1; assert(That.IsTrue(Table.hasIndices(t)))
	...
	assert(That.IsFalse(Table.hasIndices({})))
	assert(That.IsFalse(Table.hasIndices({k="x"})))
end

function tests.hasKeys()
	local t={}
	t={a=1,2,3}; assert(That.IsTrue(Table.hasKeys(t)))
	t={1,2,a=3}; assert(That.IsTrue(Table.hasKeys(t)))

	t={1,2}; assert(That.IsFalse(Table.hasKeys(t)))
	t={}; assert(That.IsFalse(Table.hasKeys(t)))
end

function tests.isContinuous()
	local t={}
	t={1,2,3}; assert(That.IsTrue(Table.isContinuous(t)))
	t={1,2,nil}; assert(That.IsTrue(Table.isContinuous(t)))
	t={1,2,a=3}; assert(That.IsTrue(Table.isContinuous(t)))
	t={a=1,2,3}; assert(That.IsTrue(Table.isContinuous(t)))

	t={nil,2,3}; assert(That.IsFalse(Table.isContinuous(t)))
	t={1,nil,3}; assert(That.IsFalse(Table.isContinuous(t)))
	t={1,2,3}; t[1]=nil; assert(That.IsFalse(Table.isContinuous(t)))
	t={1,2,3}; t[2]=nil; assert(That.IsFalse(Table.isContinuous(t)))

	t={1,2,3}; t[3]=nil; assert(That.IsTrue(Table.isContinuous(t))) --!!
end

function tests.getMaxIndex()
	local t={}
	t[1]=1; assert(That.IsEqual(Table.getMaxIndex(t),1))
	t[200]=2; assert(That.IsEqual(Table.getMaxIndex(t),200))
	t[65535]=3; assert(That.IsEqual(Table.getMaxIndex(t),65535))
	t[65535]=nil;assert(That.IsEqual(Table.getMaxIndex(t),200))
	t[500]=3; assert(That.IsEqual(Table.getMaxIndex(t),500))
end

function tests.lengthOperator()
	local t ={}
	t={1,2,3}; assert(That.IsEqual(#t,3))
	t={1,2,nil}; assert(That.IsEqual(#t,2)) -- !!
	t={1,nil,3}; assert(That.IsEqual(#t,3))
	t={nil,2,3}; assert(That.IsEqual(#t,3))

	t={1,2,3};t[3]=nil; assert(That.IsEqual(#t,2)) --!!
	t={1,2,3};t[2]=nil; assert(That.IsEqual(#t,3))
	t={1,2,3};t[1]=nil; assert(That.IsEqual(#t,3))

	t={}; t[1]=1; assert(That.IsEqual(#t,1))
	t={}; t[2]=1; assert(That.IsEqual(#t,0)) -- !!
	t={}; t[200]=1; assert(That.IsEqual(#t,0))-- !!
	t={}; t[65535]=1; assert(That.IsEqual(#t,0))-- !!

	t={nil,2,3}; t[4]=4; assert(That.IsEqual(#t,4))
	t={nil,2,3}; t[5]=5; assert(That.IsEqual(#t,0)) --!!
end
...
function tests.manipulatingArray()
	...
	assert(That.IsEqual(s,"23"))

	table.insert(a,4)
	s=""; for _, v in ipairs(a) do s=s..tostring(v) end
	assert(That.IsEqual(s,"234"))
end
