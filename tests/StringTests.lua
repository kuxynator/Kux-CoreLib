...
function tests.format()
	local n=tests["dummy"]
	assert(That.IsEqual(String.format("A%1%2","B","C"),"ABC"))
	assert(That.IsEqual(String.format("A%1",1),"A1")) -- integer
	assert(That.IsEqual(String.format("A%1",true),"Atrue"))	-- boolean
--	assert(Assert.IsEqual(String.format("A%1",n),"A"))	-- nil
	assert(That.IsEqual(String.format("A%1%2",n,"C"),"AC"))	-- nil
end

function tests.replace()
	local n=tests["dummy"]
	assert(That.IsEqual(String.replace("A{b}{c}",{b="B",c="C"}),"ABC"))	
	assert(That.IsEqual(String.replace("A{b}{c}",{b="B"}),"AB"))--key does not exist
	assert(That.IsEqual(String.replace("A{b}",{b=1}),"A1"))-- integer
	assert(That.IsEqual(String.replace("A{b}",{b=true}),"Atrue"))-- boolean
--	assert(Assert.IsEqual(String.replace("A{b}",{b=n}),"A"))-- nil
	assert(That.IsEqual(String.replace("A{b}{c}",{b=n, c="C"}),"AC"))-- nil
end

function tests.toChars()
	local chars = String.toChars("1A![$")
	--assert(Assert.Table.ContainsAll(chars,{"1","A","!","[","$"}))
	assert(That.IsEqual(chars[1],"1"))
	assert(That.IsEqual(chars[2],"A"))
	assert(That.IsEqual(chars[5],"$"))
end

function tests.escape()
	local s = "[23%.!1$]"
	assert(That.String.Contains(s,s))
	assert(That.String.ContainsAll(s,{"[","23","%",".","!","1$","]"}))
end
